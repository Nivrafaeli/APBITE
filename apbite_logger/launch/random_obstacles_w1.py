...
if __name__ == "__main__":
    ...
    for line in fr:
        if line[0]=="@":
            ...
            words=line[:-1].split(" ")
            good_dist=False
            #clean locations[] for every vertice (tsela)
            if obs_counter%3==1:
                locations = []
            while not good_dist:
                random_location=round(random.random()*1.5+1.0,2)
                good_dist=good_distance(locations,random_location)
            locations.append(random_location)
            marker_name=words[1]
            if obs_counter<=3: #the first 3 obstacles
                part3 = ".urdf -urdf   -x "
                part4 = " -y -0.0 -z 0 -Y 0 -model free_marker_"
            elif obs_counter <= 6: #the second 3 obstacles
                part3 = ".urdf -urdf   -x 2.5 -y "
                part4 = " -z 0 -Y 0 -model free_marker_"
            else: #the last 3 obstacles
                part3 = ".urdf -urdf   -x "
                part4 = " -y 2.5 -z 0 -Y 0 -model free_marker_"
                random_location=random_location-1.5
