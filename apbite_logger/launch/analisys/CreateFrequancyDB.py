...
class CreateFrequancies():
    ...
    def addFreq(self, lDict):
        ...
        for obs_num in xrange(1,11):
            #Every iteration takes obs_num targets and make a list of all their combinations
            ObsList=list(itertools.combinations(ObsNames, obs_num))
            Number_of_obs=str(obs_num)
            density=str(float(obs_num)/distance)
            for subgroup in ObsList:
                detection=0.0
...
FreqCreator=CreateFrequancies(directory_firstpart,"APBITE_FrequanciesDB.csv")
